...
| --- | --- | --- | --- | --- |
| Content Architect | Turn the creator's idea into a useful content brief, assumptions, audience, format, and smallest useful production plan. | `user_goal`, `content_context`, `available_templates` | `content_brief`, `agent_plan` | Passes the brief and plan to Orchestrator; no approval required. |
...
| Reviewer | Check drafts against goal, material, platform, creator context, and likely risks, then produce an explainable content benchmark. | `draft`, `content_brief`, `review_constraints`, `brand_profile` | `review`, `quality_report` | Recommends revisions but never silently rewrites or approves. Human decides. |
...
- Hook and benchmark outputs explain editorial choices; they do not predict or guarantee engagement.
- The runtime agents are content-production roles. Content capabilities are not exposed as a custom-agent builder interface.
